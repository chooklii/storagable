...
class Filebook extends React.Component {
    ...
    // load all File and Folder Names
    componentWillMount() {
        axios.get(FULL_ROUTE + "/filefolders", {
            headers: {
                charset: "utf-8"
            }
        })
    ...
    componentDidUpdate() {
        if (this.state.loaded == false) {
            ...
            if (this.state.current_directory) {
                url = FULL_ROUTE + "/filefolders?path=" + this.state.current_directory
            } else {
                url = FULL_ROUTE + "/filefolders"
            }
            axios.get(url, {
                headers: {
                    charset: "utf-8"
        ...
        if (this.state.photos.length > 0) {
            if (this.state.photos.length <= 14) {
                ...
                return this.state.photos.map(function (single) {
                    ...
                    if (unwanted_types.indexOf(file_ending) <= 0 || folder) {
                        image_url = FULL_ROUTE + "/Dateien/" + folder + "/" + single
                    } else {
                        image_url = FULL_ROUTE + "/file?path=" + single
                    }
                    return (
                        <div id="photobox">
